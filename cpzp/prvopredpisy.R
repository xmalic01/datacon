...
df = df %>% select(
  Id_pojistence,
  Pohlavi,
  vek_pri_prvnim_ockovani,
  vlna1,datum_predpisu,
  datum_ockovani,
  dny_do_prvniho_predpisu
)
...
ggplot(date_range) +
  ...
  labs(
    title = "ČPZP: datumy prvopředpisů",
...
# df$vlna1 = factor(df$vlna1,labels = c(0,1))
# write.csv2(df,"prvopredpisy.csv",row.names = FALSE)
...
#sloupcovej graf - nad sebou - cisla
{
  ggplot(ratios, aes(
    ...
    labs(
      title = "ČPZP: Poměry prvopředpisů",
      x = "Věková kategorie",
      y = "Poměr po / před",
      fill = ""
    ) +
...
RoR = ratios %>% 
  select(AGE_GROUP,TREATMENT,pomer) %>% 
  pivot_wider(names_from = TREATMENT, values_from = pomer) %>% 
  mutate(RoR = očkovaní / neočkovaní)

{
  ggplot(RoR,aes(x=AGE_GROUP,y=RoR,fill = AGE_GROUP))+
    geom_col(aes(group = AGE_GROUP),position = position_dodge(width = 0.8))+
    geom_hline(yintercept = 1,linetype="dashed",color="black")+
    labs(
      title = "OZP: RoR prvopředpisů",
      x = "Věková kategorie",
      y = "RoR",
      fill=""
    )+
